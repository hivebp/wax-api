...
def load_atomic_market_auct_bid(session, transaction):
    ...
    session_execute_logged(
        ...
        'SELECT :bidder, :bid, :currency, :auction_id, :taker, \'atomicmarket\', :seq, :block_num, :timestamp '
        'WHERE NOT EXISTS (SELECT seq FROM auction_bids WHERE seq = :seq)', new_sale
    )

    session_execute_logged(
        session,
        'UPDATE auctions SET current_bid = :bid '
...
def load_atomic_market_auct(session, transaction):
    ...
    session_execute_logged(
        ...
        'WHERE NOT EXISTS (SELECT seq FROM auction_logs WHERE seq = :seq)', new_sale
    )
    ...
    session_execute_logged(
        ...
        'AND NOT EXISTS (SELECT seq FROM auctions WHERE seq = :seq)', new_sale
    )
...
def update_sales_summary(session):
    ...
    session_execute_logged(
        ...
        'FROM ('
        '   SELECT collection, CASE WHEN du.new_price IS NULL THEN d.price ELSE du.new_price END AS price, '
        '   CASE WHEN du.new_currency IS NULL THEN d.currency ELSE du.new_currency END AS currency, '
