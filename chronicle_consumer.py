...
def handle_fork(block_num, unconfirmed_block, confirmed_block, session):
    try:
        ...
        removed_tables = session.execute(
            ...
            'ORDER BY ordinal_position ASC), \',\') || \' FROM \' || t.table_name || \' r\' AS query '
            'FROM tables_with_block_num t '
            ...
            session.execute(
                'INSERT INTO {table_name} {query} WHERE removed_block_num >= :block_num'.format(
                    table_name=tables['table_name'].replace(
                        ...
                        'removed_atomicmarket_auctions', 'auctions').replace(
                        'removed_simplemarket_listings', 'listings').replace(
